...
Setup sketch: create VM (Debian) → clone repo → venv + requirements → config/.env (or fetch from Secret Manager at boot) → crontab: `0 8 * * 1-5 TZ=America/New_York cd ~/trading-agent && .venv/bin/python -m tradingagent --stage all >> ~/cron.log 2>&1` → same email delivery (the `SMTP_*` block in config/.env).
